...
class BaseModel(object):
    ...
    def voxel_grid(self):
        ...
        if self._voxel_grid is None:
            try:
                ...
                # output_shape = tuple(map(int, V.split(",")))
                # if self._voxel_grid.shape != output_shape:
                #     occupancy_vox = OccupancyGrid(V)
                #     self._voxel_grid = occupancy_vox.voxelize(
                #         self.groundtruth_mesh
                #     )
        ...
        if self._occupancy_pairs is None:
            if not os.path.exists(self.path_to_occupancy_pairs):
                ...
                try:
                    ...
                    print(self.path_to_occupancy_pairs)
                    import pdb
                    pdb.set_trace()
    ...
    def _sample_points_uniform(self, N):
        ...
        points, labels = self.points_in_bbox
        weights = np.ones((N, 1), dtype=np.float32)
        if N >= (len(points) // 2):
...
class DynamicFaust(ModelCollection):
    class Model(BaseModel):
        def __init__(self, base_dir, tag, config):
            ...
            # Path to the folder containing the meshes
            self._mesh_folder = self.config.get(
                "mesh_folder", "mesh_seq_2"
            )
            # Path to the folder containing the occupancy pairs
            self._occpairs_folder = self.config.get(
                "occpairs_folder", "occpairs"
            )
        ...
        def path_to_occupancy_pairs(self):
            ...
            return os.path.join(self._base_dir, self._category,
                                self._occpairs_folder, seq)

        @property
        def path_to_surface_pairs(self):
            self._surface_points_folder = self.config["surface_points_folder"]
            return os.path.join(self._base_dir, self._category,
                                self._surface_points_folder,
                                f"{self._sequence}.npz")

        @property
        def path_to_sampled_faces(self):
            self._mesh_folder = self.config["mesh_folder"]
            return os.path.join(self._base_dir, self._category,
                                self._mesh_folder,
                                f"{self._sequence}_sampled_faces.npy")

        @property
        def image_paths(self):
            self._renderings_folder = self.config["renderings_folder"]
            return [os.path.join(self._base_dir, self._category,
                                 self._renderings_folder,
                                 f"{self._sequence}.png")]

        @property
        def path_to_voxel_grid(self):
            self._voxel_grid_folder = self.config["voxel_grid_folder"]
            voxel_grid_folder = os.path.join(
                self._base_dir, self._category, self._voxel_grid_folder
            )
            # In case the directory does not exist create it
            if not os.path.exists(voxel_grid_folder):
                os.makedirs(voxel_grid_folder)
            return os.path.join(voxel_grid_folder, f"{self._sequence}.npy")
    ...
    def __init__(self, base_dir, config):
        ...
        self._tags = sorted([
            ...
            for l in sorted(
                os.listdir(os.path.join(self._base_dir, d, mesh_folder))
            )[20:] if l.endswith(".obj")
        ])
...
class DeformingThings4D(ModelCollection):
    class Model(BaseModel):
        def __init__(self, base_dir, tag, config):
            super().__init__(tag, config)
            self._base_dir = base_dir
            self._category, self._model, self._model_idx = tag.split(":")
            # Define the path to the folder containing the data for the current
            # model
            self.path_to_model_folder = os.path.join(
                self._base_dir, self._category, self._model
            )
            self._mesh_folder = config["data"].get(
                "mesh_folder", "mesh_seq"
            )
            self._occpairs_folder = config["data"].get(
                "occpairs_folder", "occpairs_seq"
            )
            self._images_folder = config["data"].get(
                "rendereings_folder", "renderings_seq"
            )
            self._voxel_grid_folder = config["data"].get(
                "voxel_grid_folder", "voxel_grid_seq"
            )
            self._sampled_faces_folder = config["data"].get(
                "sampled_faces_folder", "sampled_faces_seq"
            )

        @property
        def path_to_mesh_file(self):
            return os.path.join(
                self._path_to_model_folder, self._mesh_folder,
                f"{self._model_idx}.obj"
            )

        @property
        def path_to_occupancy_pairs(self):
            return os.path.join(
                self._path_to_model_folder, self._occpairs_folder,
                f"{self._model_idx}.npz"
            )

        @property
        def path_to_image_paths(self):
            return [os.path.join(
                self._path_to_model_folder, self._images_folder,
                f"{self._model_idx}.png"
            )]

        @property
        def path_to_voxel_grid(self):
            return os.path.join(
                self._path_to_model_folder, self._voxel_grid_folder,
                f"{self._model_idx}.npy"
            )

        @property
        def path_to_sampled_faces(self):
            return os.path.join(
                self._path_to_model_folder, self._sampled_faces_folder,
                f"{self._model_idx}.npy"
            )
    ...
    def __init__(self, base_dir, config):
        ...
        self._categories = sorted([
            ct for ct in os.listdir(self._base_dir)
            if os.path.isdir(os.path.join(self._base_dir, ct))
        ])

        self._models = sorted([
            ml for ct in self._categories
            for ml in os.listdir(os.path.join(self._base_dir, ct))
            if os.path.isdir(os.path.join(self._base_dir, ct, ml))
        ])

        mesh_folder = config["data"].get("mesh_folder", "mesh_seq")
        self._tags = sorted([
            f"{ct}:{ml}:{tg[:-4]}" for ct in self._categories
            for ml in os.listdir(os.path.join(self._base_dir, ct))
            for tg in sorted(
                os.listdir(os.path.join(self._base_dir, ct, ml, mesh_folder))
            ) if tg.endswith(".obj")
        ])
...
class TurbosquidAnimal(ModelCollection):
    ...
    def __init__(self, base_dir, config):
        ...
        print(f"Found {len(self)} TurbosquidAnimal models")
...
class PartNet(ModelCollection):
    class Model(BaseModel):
        def __init__(self, base_dir, tag, config):
            super(PartNet.Model, self).__init__(tag, config)
            self._base_dir = base_dir

        @property
        def path_to_mesh_file(self):
            return os.path.join(self._base_dir, f"{self._tag}.obj")

        @property
        def path_to_sampled_faces(self):
            return os.path.join(self._base_dir, f"{self._tag}",
                                "sampled_faces.npy")
    ...
    def __init__(self, base_dir, config):
        ...
        self._config = config
        self._tags = sorted([
            d.split(".")[0] for d in os.listdir(self._base_dir)
            if os.path.join(self._base_dir, d).endswith(".obj")
        ])
