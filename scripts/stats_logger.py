...
class StatsLogger(object):
    ...
    def print_progress(self, epoch, batch, loss, precision="{:.5f}", cur=False, final=False):
        
        # final logging without input loss 
        if not final:
            self._loss.value = loss
        fmt = "epoch: {} - batch: {} - loss: " + precision
        
        # whether to only display the loss of current value
        if cur:
            msg = fmt.format(epoch, batch, self._loss._cur_value)
        else:
            msg = fmt.format(epoch, batch, self._loss.value)
        for k,  v in self._values.items():
            # whether to only display the loss of current value
            if cur:
                msg += " - " + k + ": " + precision.format(v._cur_value)
            else:
